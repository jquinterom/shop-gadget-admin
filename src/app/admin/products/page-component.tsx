...
import { imageUploadHandler } from "@/src/actions/categories";
import {
  createProduct,
  deleteProduct,
  updateProduct,
...
export const ProductPageComponent = ({
  ...
  const productCreateUpdateHandler = async (
    ...
    const thereAreImages = heroImageUrl && imageUrls.length > 0;
    ...
    switch (intent) {
      case "create": {
        ...
        if (thereAreImages) {
          ...
          resetAndRefresh("Product created successfully");
        }
        break;
      }
      case "update": {
        // if (heroImageUrl && imageUrls.length > 0 && slug) {
        if (thereAreImages && slug) {
          await updateProduct({
            category: Number(category),
            heroImage: heroImageUrl!,
            imagesUrl: imageUrls,
          ...
          resetAndRefresh("Product updated successfully");
        }
        break;
      }
